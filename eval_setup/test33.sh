...
tmux send-keys -t chat_server:client1 "/broadcast $MESSAGE1" Enter
...
tmux send-keys -t chat_server:client4 "/join_group CS425" Enter
tmux send-keys -t chat_server:client2 "/group_msg CS425 $MESSAGE4" Enter
...
function sanity_check {
    ...
    if [ $? != 0 ]
    then
	#echo "$expected_output %%%%%%%%% $file <------------ failed"
        echo "${TEST_NAME}: FAIL"
        # Cleanup
...
sanity_check "join" "${TEST_NAME}_client2.log"
sanity_check "join" "${TEST_NAME}_client3.log"
sanity_check "join" "${TEST_NAME}_client4.log"
sanity_check "$MESSAGE4" "${TEST_NAME}_client1.log"
sanity_check "$MESSAGE4" "${TEST_NAME}_client3.log"
